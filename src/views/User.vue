<template>
  <div class="person_page">
    <div class="person wrap">
      <!--面包屑START-->
      <Crumb :navCrumb="[{name:'首页'},{name:'个人中心'}]"></Crumb>
      <!--面包屑END-->
      <main>
        <aside>
          <div
          ...
          ></div>
          <div class="name">{{ userInfo.nickName }} <span style="cursor:pointer;" @click="loginOut">[退出]</span></div>
          <div class="title">
            <img
...
export default {
  ...
  methods: {
    // 退出
    loginOut () {
      localStorage.removeItem("x-auth-token")// 刷新页面
      this.$router.push('/home')// 去往首页
      this.$router.go(0)// 刷新当前也页面
    }
  },
